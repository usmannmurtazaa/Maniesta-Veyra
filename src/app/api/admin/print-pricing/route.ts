...
const pricingSchema = z.object({
  ...
  pricing: z.object({
    ...
    quantityDiscountTiers: z.array(z.object({ minQuantity: z.number().int(), discountPercent: z.number().min(0).max(100) })),
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!garmentId) return NextResponse.json({ error: { code: 'VALIDATION_ERROR', message: 'garmentId required' } }, { status: 400 });
    const pricing = await adminService.getPrintPricing(garmentId);
    return NextResponse.json({ data: pricing });
  } catch (error: any) {
    return NextResponse.json({ error: { code: 'INTERNAL_ERROR', message: 'Failed to fetch pricing' } }, { status: 500 });
  }
}
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ error: { code: 'INTERNAL_ERROR', message: 'Failed to update pricing' } }, { status: 500 });
  }
}